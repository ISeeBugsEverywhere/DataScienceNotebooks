...
class properties_of:
    def __init__(self, name, engine="sqlite_memory", db_path=None):
        """
    ...
    def get_all_properties(self):
        """Gaunamos visos savybės visiems objektams."""
        if self.engine == "pandas":
            return self.df_property.merge(self.df_property_type, on="property_id", how="left")
        elif self.engine == "pyspark":
            return self.df_property.join(self.df_property_type, "property_id", "left")
        else:
            self.cursor.execute(f"""
...
def main():
    ...
    obj_memory.add_property("555-222-333", "title", "Mėnuo Juodaragis")
    obj_memory.add_property("777-222-333", "title", "Meškiukas")
    ...
    obj_memory.add_property_type("metai", "Išleidimo metai")
    obj_pandas.add_property("222-333-111", "title", "Lapė Snapė")
    obj_pandas.add_property("222-333-111", "title", "Mėnuo Juodaragis")
    obj_pandas.add_property("222-333-111", "title", "Meškiukas")
