...
def geo2grid(lat, lon):
    ...
    Args:
        lat (float): WGS-84 lat in DD format, float
        lon (float): WGS-84 long in DD format, float
...
# LKS-94 to WGS-84
def grid2geo(lat, lon):
    """LKS-94 į WGS-84

    Args:
        x (int): LKS94 koordinatė  /platuma turėtų būti
        y (int): LKS94 koordinatė y /ilguma turėtų būti

    Returns:
        lat, lon : WGS84 koordinatės DD formatu (latitude, longitude, platuma, ilguma)
    """
    x = lat
    y = lon
    k = 0.9998
